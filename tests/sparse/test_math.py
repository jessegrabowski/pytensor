...
class TestStructuredDot:
    ...
    def test_csc_correct_output_faster_than_scipy(self):
        ...
        for M, N, K, nnz in [
            ...
            if pytensor.config.mode == "FAST_RUN" and pytensor.config.cxx:
                assert pytensor_time <= 1.2 * scipy_time + 0.003
    ...
    def test_csr_correct_output_faster_than_scipy(self):
        ...
        for M, N, K, nnz in [
            ...
            utt.assert_allclose(scipy_result, pytensor_result)
            if pytensor.config.mode == "FAST_RUN" and pytensor.config.cxx:
                assert pytensor_time <= 1.1 * scipy_time + 0.002
    ...
    def test_basic(self):
        def mat(format, name, dtype):
            if format == "dense":
                ...
                return matrix(format, name, dtype=dtype)
        ...
        for dtype1, dtype2, dtype3, dtype4, format1, format2 in params:
            ...
            updates = [(z, z - a * structured_dot(x, y))]
            ...
            if not pytensor.config.blas__ldflags:
                ...
                assert isinstance(topo[0].op, Dot)
                assert isinstance(topo[1].op, DimShuffle)
                assert isinstance(topo[2].op, Elemwise)
                assert isinstance(topo[3].op, Elemwise)
            elif (
                format1 == "csc"
                and format2 == "dense"
                and not fast_compile
                and pytensor.config.cxx
                and pytensor.scalar.upcast(dtype1, dtype2, dtype3, dtype4)
                in ("float32", "float64")
            ):
                pass
                ...
                assert isinstance(topo[2].op, Usmm)
    ...
    def test_infer_shape(self):
        ...
        for dtype1, dtype2, dtype3, dtype4, format1, format2 in params:
            ...
            f_shape = pytensor.function(
                [a, x, y], (z - a * structured_dot(x, y)).shape, mode=mode
            )
            assert all(f_shape(a_data, x_data, y_data) == (self.z.shape))
